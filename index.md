...
- Fillable **checklists** for a subset of guidelines: a small
  hand-verified core (currently PRISMA 2020, CONSORT and STROBE) plus
  best-effort, automatically extracted checklists for many more, each
  item carrying its provenance and a parse-confidence score. Coverage is
  partial and not all extracted checklists are verified;
  [`reportilo_coverage()`](https://choxos.github.io/reportilo/reference/reportilo_coverage.md)
  reports what is verified versus extracted, by category.
- Data-driven **flow diagram** templates: PRISMA 2020, CONSORT and
  STARD, plus observational designs (cohort, case-control and
  cross-sectional).
...
available as catalog entries that link to their original source. Most
checklists are extracted automatically and should be verified against
the source before use; only a small core is hand-verified. See
[`reportilo_coverage()`](https://choxos.github.io/reportilo/reference/reportilo_coverage.md)
for verified-versus-extracted coverage by category, and `parse_status`
for per-guideline detail.
